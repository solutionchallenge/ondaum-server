...
* **주의사항:** 만약 전체 대화 기록이 너무 짧아서 100자 이상 분석 내용을 작성하기 힘든 경우, 대화가 너무 짧아서 분석이 힘들다는 것을 솔직하게 언급하며 더 많은 대화를 유도하고 격려하는 내용을 부드러운 어조로 적습니다.
...
* **요구사항:**
    ...
    * 만약 전체 대화 기록이 너무 짧아서 키워드를 추출할 수 없다면, 이 배열은 비워둘 수 있습니다.

### 4. `emotions` ([]struct)

* **내용:** 대화 전체에서 드러난 사용자의 주요 감정을 분석하여, 아래 명시된 7가지 감정 범주에 대한 비율을 추정합니다.
    * **감정 범주 (Emotion string):** `"joy"`, `"sadness"`, `"anger"`, `"surprise"`, `"fear"`, `"disgust"` (이 중 해당하는 감정만 포함)
...
* **요구사항:**
    ...
    * 감정을 하나도 포함하지 않거나, 모든 감정의 스코어를 0.0으로 줄 수는 없습니다. **최소한 하나의 감정은 0.10 이상의 값으로 포함**해야 합니다.
